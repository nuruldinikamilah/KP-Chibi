		...
		<div id="sidebar" class="sidebar">
			...
			<div data-scrollbar="true" data-height="100%">
				...
				if (isset($_GET['rl'])) {
					...
					} else if ($data[1] == 'kaprodi') {
						$sql = mysqli_query($server1, "SELECT
													*
													FROM
													kaprodi
													WHERE nip_kaprodi='" . $_SESSION['nik_user'] . "'");
						$r = mysqli_fetch_array($sql);
						if ($data[0] == $r['random_id']) {
							call_menu_kaprodi();
						} else {
					...
					} else if ($data[1] == 'dekan') {
						$sql = mysqli_query($server1, "SELECT
													*
													FROM
													dekan
													WHERE nip_dekan='" . $_SESSION['nik_user'] . "'");
						$r = mysqli_fetch_array($sql);
						if ($data[0] == $r['random_id']) {
							call_menu_dekan();
						} else {
