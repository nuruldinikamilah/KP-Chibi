...
class OLERead {
	...
	function read($sFileName){
		...
		if (!$this->data) {
   		...
   		if (mb_substr($this->data, 0, 8, '8bit') != IDENTIFIER_OLE) {
			$this->error = 1;
			return false;
   		}
	...
	function __readData($bl) {
		...
		while ($block != -2)  {
			...
			$data = $data.mb_substr($this->data, $pos, BIG_BLOCK_SIZE, '8bit');
	...
	function __readPropertySets(){
		...
		while ($offset < strlen($this->entry)) {
			$d = mb_substr($this->entry, $offset, PROPERTY_STORAGE_BLOCK_SIZE, '8bit');
	...
	function getWorkBook(){
		if ($this->props[$this->wrkbook]['size'] < SMALL_BLOCK_THRESHOLD){
			...
			while ($block != -2) {
				  ...
				  $streamData .= mb_substr($rootdata, $pos, SMALL_BLOCK_SIZE, '8bit');
			...
			while ($block != -2) {
			  $pos = ($block + 1) * BIG_BLOCK_SIZE;
			  $streamData .= mb_substr($this->data, $pos, BIG_BLOCK_SIZE, '8bit');
			  $block = $this->bigBlockChain[$block];
			}
			return $streamData;
		}
...
class Spreadsheet_Excel_Reader {
	...
	function read16bitstring($data, $start) {
		...
		return mb_substr($data, $start, $len, '8bit');
	}
			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						if ($spos + $len < $limitpos) {
							$retstr = mb_substr($data, $spos, $len, '8bit');
							$spos += $len;
						}
						else{
							// found countinue
							$retstr = mb_substr($data, $spos, $limitpos - $spos, '8bit');
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
					...
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
						...
						if (ord($data[$pos+8]) == 0){
							$formatString = mb_substr($data, $pos+9, $numchars, '8bit');
						} else {
							$formatString = mb_substr($data, $pos+9, $numchars*2, '8bit');
						}
					} else {
						$numchars = ord($data[$pos+6]);
						$formatString = mb_substr($data, $pos+7, $numchars*2, '8bit');
					}
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FONT:
						...
						if ((ord($data[$pos+19]) & 1) == 0){
						    $font = mb_substr($data, $pos+20, $numchars, '8bit');
						} else {
						    $font = mb_substr($data, $pos+20, $numchars*2, '8bit');
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						...
						if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
							...
							if ($chartype == 0){
								$rec_name	= mb_substr($data, $pos+12, $rec_length, '8bit');
							} else {
								$rec_name	= $this->_encodeUTF16(mb_substr($data, $pos+12, $rec_length*2, '8bit'));
							}
						}elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7){
								$rec_name	= mb_substr($data, $pos+11, $rec_length, '8bit');
						}
		...
		while($cont) {
			...
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$tmp = unpack("ddouble", mb_substr($data, $spos + 6, 8, '8bit')); // It machine machine dependent
					if ($this->isDate($spos)) {
						$numValue = $tmp['double'];
					}
					else {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_STRING:
					...
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
						...
						$len = ($asciiEncoding)?$numChars : $numChars*2;
						$retstr =mb_substr($data, $xpos, $len, '8bit');
						$xpos += $len;
						$retstr = ($asciiEncoding)? $retstr : $this->_encodeUTF16($retstr);
					}
					elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7){
						...
						$numChars =ord($data[$xpos]) | (ord($data[$xpos+1]) << 8);
						$xpos += 2;
						$retstr =mb_substr($data, $xpos, $numChars, '8bit');
					}
				...
				case SPREADSHEET_EXCEL_READER_TYPE_LABEL:
					$row	= ord($data[$spos]) | ord($data[$spos+1])<<8;
					$column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
					$this->addcell($row, $column, mb_substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7])<<8, '8bit'));
					break;
				...
				case SPREADSHEET_EXCEL_READER_TYPE_HYPER:
					...
					if (($flags & 1) > 0 ) {   // is a type we understand
						...
						if (($flags & 0x14) == 0x14 ) {   // has a description
							...
							$udesc = mb_substr($this->data, $spos + $uloc, $descLen * 2, '8bit');
							$uloc += 2 * $descLen;
						}
