          ...
          $('input[type=radio][name=answer_9]').change(function() 
        {
         ...
         //simpan hasil akhir
          ...
          //$('#hasil_akhir').val(hasil_akhir);
           ...
           var table_4 = $('#tabelinformasiditerima').DataTable( {
               scrollY:        "300px",
               scrollX:        true,
                scrollCollapse: true,
                paging:         false,
                ...
                "oLanguage": 
                    {
                       "sSearch": "Pencarian Penelitian:",
                        "sLengthMenu": "Tampilkan Sebanyak _MENU_ Data"
                     },
                  order: [[ 1, "asc" ]]
                    });

           //clear header
     $('#pengajuan_koreksi').on('shown.bs.modal', function (e) {
      /* setTimeout(function () 
       {
...
if ($_POST['act']=='pengajuan_koreksi')
{
  ...
  <?php
  $sql_get_id_sister=mysqli_query($server1,"SELECT
                                                    *
                                                    FROM
                                                    ...
                                                    WHERE a.idx_penelitian='".$data_id."'");
   while($rx = mysqli_fetch_array($sql_get_id_sister))
    {
      $id_sister[]=$rx['id_sister'];
      $status_peneliti[]=$rx['status_peneliti'];
    }


    $sql_get_publikasi_sister=mysqli_query($server1,"SELECT
                                                        YEAR(c.`tanggal_terbit`) AS tahun,
                                                        e.`status_peneliti`,
                                                        c.`judul_publikasi_paten`,
                                                        c.`jenis_publikasi`,
                                                        c.`tanggal_terbit`,
                                                        c.`jurnal`,
                                                        c.`penerbit`
                                                        ...
                                                        sister.`dosen` a
                                                        INNER JOIN
                                                        sister.`publikasi_dosen` b
                                                        INNER JOIN
                                                        sister.`publikasi` c
                                                        INNER JOIN
                                                        `integrasi`.`karyawan` d
                                                        INNER JOIN
                                                         `lppm2020`.`pengajuan_anggota_penelitian` e
                                                        ON a.`id_dosen` = b.`id_dosen` AND
                                                           b.`id_riwayat_publikasi_paten` = c.`id_riwayat_publikasi_paten` AND
                                                           d.`id_sister` = a.`id_dosen` AND
                                                           d.`nip` = e.`nip_anggota`
                                                        WHERE a.id_dosen='".$id_sister[0]."' OR a.`id_dosen`='".$id_sister[1]."'
                                                        GROUP BY `judul_publikasi_paten` order by c.`tanggal_terbit` desc");
  ?>
 
<table id="tabelinformasiditerima" class="table table-striped table-bordered" >
    <thead>
        <tr>
                        <td class="width:100%" colspan="6">
                          <center><b><span class="blink"><font color="red"><b>Rekam Jejak Ketua dan Anggota Peneliti 
                       </td>
                    </td>
                 </tr>
        <tr>
            <th>Tahun</th>
            <th>Status Peneliti</th>
    ...
    <tbody>
      <?php
      while($ry = mysqli_fetch_array($sql_get_publikasi_sister))
       {
        ?>
          <tr>
            <td><?php echo $ry['tahun'];?></td>
            <td><?php echo $ry['status_peneliti'];?></td>
            <td><?php echo $ry['judul_publikasi_paten'];?></td>
             <td><?php echo $ry['jenis_publikasi'];?></td>
           <td><?php echo $ry['jurnal'];?></td>
            <td><?php echo $ry['penerbit'];?></td>
        </tr>
        <?php
         //echo $ry['judul_publikasi_paten'];
       }
      ?>
    </tbody>
</table>
            ...
            <input type="hidden" value="<?php echo $jumlah_data;?>" name="jumlah_data">
               <tr>
                   ...
                   <input type="hidden" name="idx_nilai_1" value="<?php echo $idx_penilaian[0];?>">
                    ...
                    <td class="text-left">1 = Sangat Tidak Setuju, 2 = Tidak Setuju, 3 = Ragu-Ragu, 4 = Setuju, 5 = Sangat Setuju</td>
                 </tr>
                <tr>
                    <td>2</td>
