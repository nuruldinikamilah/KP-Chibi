    ...
    if ($_SESSION['nik_user']=='')
    {
  	...
  	else
  	{
  		   ...
  		   $result=mysqli_query($server1,"UPDATE `pengajuan_pengabdian` SET `validasi_proposal_pengguna` = 'y' , `tgl_validasi_proposal_pengguna` = now() WHERE `idx_pengabdian` = '".$idx."' ");
                 ...
                 {
                   //REDIRECT
                  header('location:../../view.php?menu=pengabdian&act=usulan_baru_langkah_lima&idx='.$_POST['idx'].'&status=sukses');
                 }
                 else
                 {
                 //REDIRECT
                  header('location:../../view.php?menu=pengabdian&act=usulan_baru_langkah_lima&status=gagal&kode='.mysqli_error($server1));
                 }  
