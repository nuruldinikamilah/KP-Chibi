<?php
       ...
       header("Content-type: application/vnd-ms-excel");
        ...
        $sekarang = date('d-m-Y');
       header("Content-Disposition: attachment; filename=Laporan Penelitian Pencarian Internal-".$sekarang.".xls");
        ...
        if ($_GET['nip_reviewer'] != "")
        {
            ...
            {
                $kondisiwhere .= "where a.nip_reviewer=".$_GET['nip_reviewer'];
...
<?php
        ...
        <table border="80">
            <tr>
                                        <th width="2%">No.</th><th width="2%">Tahun Pengajuan</th><th>Judul</th><th>Skema</th><th>NIP Ketua</th><th>Personil Peneliti</th><th>Nama Reviewer</th><th>Nilai Proposal</th><th width="40%">Catatan Reviewer</th>
                                    </tr>
            <?php   
            //query get data
                 ...
                 if (is_null($data['nilai_keseluruhan_proposal']) || is_null($data['catatan_reviewer']))
                  {
                    $nilai=0;
                       echo "<tr  bgcolor = 'red'>";
                        echo "<td style='width:2%;vertical-align: top;' align='center'>".$no."</td>";
                  ...
                  else
                  {
                        ...
                        echo "<tr>";
                           ...
                           ?>
                           <?php
                           $sql_anggota=mysqli_query($server1,"select * from pengajuan_anggota_penelitian where idx_penelitian=".$data['idx_penelitian']." order by status_peneliti desc");
