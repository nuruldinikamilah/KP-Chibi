<?php
 ...
 if(isset($_POST['submit'])) 
 {
	for ($x = 1; $x <= $_POST['jumlah_data']; $x++) 
	{
	  ...
	  $result = mysqli_query($server1,"UPDATE `pengajuan_pengabdian` SET `nilai_laporan_kemajuan` = '".$_POST['hasil_akhir']."', catatan_reviewer_kemajuan = '".$_POST['komentar']."' WHERE `idx_pengabdian` = '".$idx_pengabdian."'");

	 if($result)
        {
            //REDIRECT
            header('location:../../view.php?menu=laporan_kemajuan&act=daftar_kemajuan_pengabdian_rev&status=sukses');
        }
        else
        {
            //REDIRECT
            header('location:../../view.php?menu=laporan_kemajuan&act=daftar_kemajuan_pengabdian_rev&status=gagal&kode='.mysqli_error());
